...
//        $nameXML = preg_replace("/[\r\n|\n|\r]+/", "", $xml);
    ...
    protected function zipBase64(
        ...
        $nameXML = preg_replace(
            "/[\r\n|\n|\r]+/",
            "",
            $this->getFileName($company, $resolution)
        );
        ...
        if ($batch) {
            ...
            $nameZip = preg_replace(
                "/[\r\n|\n|\r]+/",
                "",
                $this->getFileName($company, $resolution, 6, '.zip')
            );
        }
        ...
        $this->pathZIP = preg_replace(
            "/[\r\n|\n|\r]+/",
            "",
            "app/zip/{$resolution->company_id}/{$nameZip}"
        );

        // Guardar XML temporal
        Storage::disk('local')->put(
            "xml/{$resolution->company_id}/{$nameXML}",
            $sign->xml
        );
        ...
        $result_code = $zip->open(
            storage_path($this->pathZIP),
            ZipArchive::CREATE
        );
        ...
        if ($result_code !== true) {
            ...
            $zip->add_file(
                file_get_contents(
                    storage_path("app/xml/{$resolution->company_id}/{$nameXML}")
                ),
                $nameXML
            );
            ...
            // Guardar local o Spaces
            if ($GuardarEn) {

                $directory = dirname($GuardarEn);

                if (!file_exists($directory)) {
                    mkdir($directory, 0777, true);
                }

                file_put_contents($GuardarEn . ".zip", $zipContent);

            } else {

                Storage::disk('do_spaces')->put(
                    "zip/{$resolution->company_id}/{$nameZip}",
                    $zipContent,
                    'public'
                );
            }
        ...
        if ($GuardarEn) {
            ...
            file_put_contents($GuardarEn . ".xml", $sign->xml);
    ...
    protected function saveAnnexes($annexes, $filename)
    {
        ...
        foreach($annexes as $document){
            $i++;
            dd("public/{$company->identification_number}/{$document_filename}");
